...
<html lang="fr" xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body class="body" style="margin:0; padding:0; width:100%; background-color:#f6f7f9; -webkit-text-size-adjust:100%; -ms-text-size-adjust:100%;">
    ...
    <div style="display:none; max-height:0; overflow:hidden; mso-hide:all; font-size:1px; line-height:1px; color:#f6f7f9;">
      Nouveau message de ${escapeHTML(name)} — ${escapeHTML(company)}
    </div>
                          ...
                          <td style="padding:14px;">
                            ...
                            <p class="p" style="margin:0; font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; font-size:15px; line-height:22px; font-weight:600; word-break:break-all;">
                              <a href="mailto:${escapeHTML(
                                email
                              )}" style="color:${brand}; text-decoration:none;">${escapeHTML(
    email
  )}</a>
                            </p>
                          </td>
                      ...
                      <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
                        <tr>
                          <td style="padding-bottom:12px;">
                            <a href="mailto:${escapeHTML(
                              email
                            )}" class="btn-primary" style="display:block; background:${cta}; color:#ffffff; text-align:center; padding:14px 24px; border-radius:8px; text-decoration:none; font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; font-size:15px; font-weight:700; line-height:20px;">
                              Répondre par email
                            </a>
                          </td>
                        </tr>
                        <tr>
                          <td>
                            <a href="tel:${escapeHTML(
                              phone
                            )}" class="btn-primary" style="display:block; background:${brand}; color:#ffffff; text-align:center; padding:14px 24px; border-radius:8px; text-decoration:none; font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; font-size:15px; font-weight:700; line-height:20px;">
                              Appeler maintenant
                            </a>
                          </td>
                        </tr>
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!checkRateLimit(`contact_ip_${clientIP}`, 3, 60 * 60 * 1000)) {
      auditLogger.log({
        action: "RATE_LIMIT_EXCEEDED",
        ip: clientIP,
        userAgent: request.headers.get("user-agent") || "",
        details: { endpoint: "contact", scope: "ip" },
      });
    ...
    const { name, email, phone, service, details } = body || ({} as ContactFormData);
    ...
    if (!checkRateLimit(`contact_email_${sanitizedEmail}`, 3, 60 * 60 * 1000)) {
      auditLogger.log({
        action: "RATE_LIMIT_EXCEEDED",
        ip: clientIP,
        userAgent: request.headers.get("user-agent") || "",
        details: { endpoint: "contact", scope: "email", email: sanitizedEmail },
      });
      return NextResponse.json(
        { error: "Trop de soumissions, veuillez réessayer plus tard." },
        { status: 429 }
    ...
    const subject = `Nouveau message de ${sanitizedName}${sanitizedService ? ` — ${sanitizedService}` : ""}`;
    ...
    const detailsHTML =
      sanitizedDetails && sanitizedDetails.trim().length
        ? nl2br(escapeHTML(sanitizedDetails))
        : undefined;
...
${sanitizedService ? `• Sujet : ${sanitizedService}\n` : ""}
${sanitizedDetails ? `\nMESSAGE\n${sanitizedDetails}\n` : ""}
